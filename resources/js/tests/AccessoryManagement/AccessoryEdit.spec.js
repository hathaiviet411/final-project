...
import AccessoryEdit from '@/pages/AccessoryManagement/Edit';

describe('TEST PAGE ACCESSORY EDIT', () => {
    const backToAccessoryIndex = jest.fn();
    const validation = jest.fn();
    const updateAccessory = jest.fn();
    const getOneAccessory = jest.fn();
    ...
    const wrapper = mount(AccessoryEdit, {
        localVue,
        router,
        store,
        mocks,
    ...
    test('Test component render form edit', async() => {
        const AccessoryEdit = wrapper.find('.accessory-management-edit');
        expect(AccessoryEdit.exists()).toBe(true);

        const Content = AccessoryEdit.find('.content-body');
        ...
        const ButtonUpdate = ButtonFunction.find('button#btn-save');
        expect(ButtonUpdate.exists()).toBe(true);
        expect(ButtonUpdate.classes('btn')).toBe(true);
        expect(ButtonUpdate.classes('btn-secondary')).toBe(true);
        expect(ButtonUpdate.classes('v-button-default')).toBe(true);
        expect(ButtonUpdate.text()).toEqual('BUTTON.REGISTER');
    ...
    test('Test component run function validation with not fill accessory name', async() => {
        setTimeout(async() => {
            const ButtonUpdate = wrapper.find('button#btn-save');
            ...
            expect(updateAccessory).not.toHaveBeenCalled();
        }, 1000);
    });
    ...
    test('Test component run function validation with full data', async() => {
        setTimeout(async() => {
            ...
            expect(validation).toHaveBeenCalled();
            expect(updateAccessory).toHaveBeenCalled();
        }, 1000);
    });

    test('Test default accessory data', async() => {
        ...
        await wrapper.vm.getOneAccessory();
        expect(getOneAccessory).toHaveBeenCalled();

        expect(wrapper.vm.accessory_name).not.toBeNull();
        expect(wrapper.vm.tonnage).not.toBeNull();
        expect(wrapper.vm.passed_year).not.toBeNull();
        expect(wrapper.vm.mileage).not.toBeNull();
        expect(wrapper.vm.remark).not.toBeNull();
    });
});
