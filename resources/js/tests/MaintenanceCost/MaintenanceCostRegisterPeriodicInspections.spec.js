...
import MaintenanceCostRegisterPeriodicInspections from '@/pages/MaintenanceCost/PeriodicInspections';

describe('TEST COMPONENT MAINTENANCE COST REGISTER PERIODIC INSPECTIONS', () => {
    const mocks = {
        $bus: {
            on: jest.fn(),
            once: jest.fn(),
    ...
    const handleGetMaintenanceCost = jest.fn();
    ...
    const wrapper = mount(MaintenanceCostRegisterPeriodicInspections, {
        localVue,
        router,
        store,
        mocks,
        methods: {
            handleGetMaintenanceCost,
    ...
    test('Test component init data', async() => {
        expect(handleGetMaintenanceCost).toHaveBeenCalled();
